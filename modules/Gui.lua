...
local Title = Instance.new("TextLabel", Header); Title.Text = "  RIVALS | SHADOW ELITE v12.0"; Title.Size = UDim2.new(1, 0, 1, 0); Title.TextColor3 = Color3.new(1,1,1); Title.Font = Enum.Font.GothamBold; Title.TextSize = 14; Title.TextXAlignment = Enum.TextXAlignment.Left; Title.BackgroundTransparency = 1
...
local function AddSlider(text, min, max, key)
    ...
    local function Update(input) local rel = math.clamp((input.Position.X - SliderBg.AbsolutePosition.X) / SliderBg.AbsoluteSize.X, 0, 1); local val = math.floor((min + (max - min) * rel) * 100) / 100; SliderFill.Size = UDim2.new(rel, 0, 1, 0); SLabel.Text = "  " .. text .. ": " .. val; _G.SETTINGS[key] = val end
    SliderBg.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then local move; move = UIS.InputChanged:Connect(function(m) if m.UserInputType == Enum.UserInputType.MouseMovement or m.UserInputType == Enum.UserInputType.Touch then Update(m) end end); local endc; endc = UIS.InputEnded:Connect(function(e) if e.UserInputType == Enum.UserInputType.MouseButton1 or e.UserInputType == Enum.UserInputType.Touch then move:Disconnect(); endc:Disconnect() end end); Update(i) end end)
end
...
AddToggle("Stretched Res (4:3)", "StretchedRes") -- NOWE
